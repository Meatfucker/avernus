...
from diffusers import (StableDiffusionImg2ImgPipeline, DPMSolverMultistepScheduler, DDIMScheduler, DDPMScheduler,
...
avernus_sd15 = FastAPI()

def load_sd15_pipeline(model_name):
    global PIPELINE
...
def sd15_generate(data: SD15Request = Body(...)):
    ...
    try:
        ...
        return {"status": False,
                "status_message": str(e)}
    return {"status": True,
            "status_message": "SD15 Success",
            "images": base64_images}

@avernus_sd15.get("/online")
async def status():
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run(avernus_sd15, host="0.0.0.0", port=6970, log_level="critical")