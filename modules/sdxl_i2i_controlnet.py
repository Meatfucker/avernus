...
import cv2
from diffusers import (StableDiffusionXLControlNetImg2ImgPipeline, DPMSolverMultistepScheduler, DDIMScheduler,
                       DDPMScheduler, LMSDiscreteScheduler, EulerDiscreteScheduler, HeunDiscreteScheduler,
...
def load_sdxl_controlnet_i2i_pipeline(model_name, controlnet_processor):
    global PIPELINE
    controlnet = get_sdxl_controlnet(controlnet_processor)
    PIPELINE = StableDiffusionXLControlNetImg2ImgPipeline.from_pretrained(model_name,
...
def generate_sdxl(prompt,
    ...
    global LOADED
    if not LOADED:
        load_sdxl_controlnet_i2i_pipeline(model_name, controlnet_processor)
        LOADED = True
