...
from diffusers import (StableDiffusionInpaintPipeline, DPMSolverMultistepScheduler, DDIMScheduler, DDPMScheduler,
...
from pydantic_models import SD15InpaintRequest, ImageResponse
from utils import base64_to_image, image_to_base64, get_seed_generators, load_loras

PIPELINE: StableDiffusionInpaintPipeline
LOADED: bool = False
dtype = torch.bfloat16
avernus_sd15_inpaint = FastAPI()

def load_sd15_inpaint_pipeline(model_name):
    global PIPELINE
...
def generate_sd15_inpaint(prompt,
                          negative_prompt,
                          width,
                          height,
                          steps,
    ...
    if model_name is None:
        model_name = "digiplay/DreamShaper_8"
    global LOADED
    if not LOADED:
        load_sd15_inpaint_pipeline(model_name)
        LOADED = True
    kwargs = {}
...
@avernus_sd15_inpaint.post("/sd15_inpaint_generate", response_model=ImageResponse)
def sdxl_inpaint_generate(data: SD15InpaintRequest = Body(...)):
    ...
    try:
        response = generate_sd15_inpaint(**kwargs)
...
@avernus_sd15_inpaint.get("/online")
async def status():
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run(avernus_sd15_inpaint, host="0.0.0.0", port=6970, log_level="critical")